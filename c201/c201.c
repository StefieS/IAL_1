...
void List_InsertFirst( List *list, int data ) {
	ListElementPtr buffer = malloc(sizeof(struct ListElement));
	if (buffer == NULL) {
		List_Error();
		return;
	}
...
void List_DeleteFirst( List *list ) {
	...
	ListElementPtr buffer = list->firstElement;
	list->firstElement = list->firstElement->nextElement;
	if (buffer == list->activeElement) {
			list->activeElement = NULL;
		}
	free (buffer);
	list->currentLength--;
...
void List_DeleteAfter( List *list ) {
	...
	ListElementPtr buffer = list->activeElement->nextElement;
